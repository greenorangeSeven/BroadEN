...
- (void)viewDidLoad
{
    ...
    {
        doneBarButton = [[UIBarButtonItem alloc] initWithTitle:@"完成" style:UIBarButtonItemStyleDone target:self action:@selector(doneAction:)];
    ...
    UILabel *tile = (UILabel *)self.navigationItem.titleView;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        if([self.selectedAssets count] > self.pickCount)
        {
            [Tool showCustomHUD:[NSString stringWithFormat:@"最多只能选择%li张图片",(long)self.pickCount] andView:self.view andImage:nil andAfterDelay:1.2f];
            [self.selectedAssets removeIndex:indexPath.row];
            return;
        }
        ...
        [UIView animateWithDuration:0.2 delay:0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
            ...
            if ([self.selectedAssets count])
            {
                ...
                {
                    self.title = [NSString stringWithFormat:@"%lu %@ 选中",(unsigned long)[self.selectedAssets count],@"张图片"];
                }
