...
#import "SGSheetMultiMenu.h"
#import <QuartzCore/QuartzCore.h>

#define kMAX_SHEET_TABLE_HEIGHT   400

@interface SGSheetMultiMenu () <UITableViewDataSource, UITableViewDelegate>
...
@implementation SGSheetMultiMenu
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        if(!self.selectedItemIndexs)
        {
            self.selectedItemIndexs = [[NSMutableDictionary alloc] init];
        }
...
- (void)triggerSelectedAction:(void (^)(NSMutableDictionary *selectedDic))actionHandle
{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSNumber *index = self.selectedItemIndexs[[NSString stringWithFormat:@"%li",indexPath.row]];
    if (index && index.intValue == indexPath.row)
    {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (index && index.intValue == indexPath.row)
    {
        [self.selectedItemIndexs removeObjectForKey:[NSString stringWithFormat:@"%li",indexPath.row]];
        [tableView reloadData];
    }
    else
    {
        NSNumber *number = [NSNumber numberWithInteger:indexPath.row];
        [self.selectedItemIndexs setObject:number forKey:[NSString stringWithFormat:@"%li",indexPath.row]];
        [tableView reloadData];
    }
