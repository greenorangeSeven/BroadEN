...
//    }else{
    ...
    if (!CGRectContainsPoint(menu.frame, touchPoint))
    {
        ...
        //如果为多选则需要在推出时执行
        if([menu isKindOfClass:[SGSheetMultiMenu class]])
        {
            SGSheetMultiMenu *mmenu = (SGSheetMultiMenu *)menu;
            mmenu.actionHandle(mmenu.selectedItemIndexs);
        }
...
+ (void)showSheetMultiWithTitle:(NSString *)title
                itemTitles:(NSArray *)itemTitles
             itemSubTitles:(NSArray *)itemSubTitles
             selectedDicIndex:(NSMutableDictionary *)selectedDicIndexs
            selectedHandle:(SGMenuMultiActionHandler)handler{
    SGSheetMultiMenu *menu = [[SGSheetMultiMenu alloc] initWithTitle:title
                                                itemTitles:itemTitles
                                                 subTitles:itemSubTitles];
    menu.selectedItemIndexs = selectedDicIndexs;
    [menu triggerSelectedAction:^(NSMutableDictionary *index){
        [[SGActionView sharedActionView] dismissMenu:menu Animated:YES];
        if (handler)
        {
            handler(index);
